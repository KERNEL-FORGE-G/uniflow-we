...
## E2E UY1, messagerie et charge — 23 août 2026

La suite `pnpm run test:e2e:uy1` a terminé avec `passed=true` et `cleanup.ok=true`. Elle crée des comptes UY1 / ICT4D / L1 éphémères, confirme les dix inscriptions académiques, exécute l’appel, persiste puis relit une note, et supprime ensuite tous les comptes, documents, conversations, messages, statuts et plans QA utilisés pendant le scénario.

La messagerie est vérifiée de bout en bout côté Appwrite : la conversation déterministe est créée entre deux comptes universitaires, un message est écrit dans `chat_messages`, puis le destinataire le relit avec ses propres permissions. Cette validation porte sur la persistance et le contrôle d’accès des Functions ; la vérification ergonomique depuis l’interface de production reste à effectuer après publication.

Les rafales idempotentes de `attendance_secure` ont toutes abouti : une requête a atteint un p50 client de **16 733,71 ms** (durée Function 3,09 s), quatre requêtes ont atteint un p50 de **8 885,64 ms** et un p95 de **15 408,39 ms** (p95 Function 3,40 s), et huit requêtes ont atteint un p50 de **13 549,80 ms** et un p95 de **26 455,71 ms** (p95 Function 3,30 s). Aucun échec n’a été constaté et l’audit final a maintenu exactement un relevé pour l’apprenant et la séance de test. La différence entre temps client et durée Function demeure cohérente avec l’attente des exécutions asynchrones et les démarrages à froid ; les assertions E2E vérifient donc en priorité les effets persistés.

## Paiement WhatsApp traçable — 23 août 2026

La collection `subscription_payment_requests` est provisionnée dans Appwrite avec une référence unique, le plan et son instantané de prix, le cycle, le demandeur, l’état `PENDING`, les horodatages, l’administrateur de traitement et une note de décision. La Function `subscription_payments` (déploiement prêt `6a8b3f17d4455122dd04`) utilise une clé serveur protégée ; elle ne fait jamais transiter la confirmation par le navigateur.

Le test E2E a créé un plan QA isolé, persisté une demande à l’état `PENDING` avec une référence `UF-…`, confirmé l’absence d’envoi WhatsApp automatique, puis appelé la décision `CONFIRMED` depuis un compte `ADMIN`. Cette décision a activé le statut Appwrite de l’apprenant ; le plan, la demande, le statut et les comptes QA ont été supprimés durant le nettoyage. L’ouverture WhatsApp est un lien prérempli vers le numéro opérationnel communiqué et la réception/vérification de la preuve demeure volontairement manuelle : aucun clic ne peut activer l’abonnement.

## Contrôle d’interface WhatsApp — 24 août 2026

Depuis `#/pricing`, la production affiche la formule indépendante **UniFlow Personnel** à **100 FCFA mensuels** et la formule académique UY1 incluse. Un compte QA indépendant créé via l’interface a franchi les étapes de souscription et a obtenu une référence unique à l’état `PENDING`; la page de confirmation a affiché le montant, le statut Appwrite et une URL `wa.me` préremplie vers le numéro communiqué. Cette URL a été inspectée sans être ouverte ni utilisée pour envoyer un message.

L’espace `#/admin/paiements` a ensuite affiché la même demande QA dans la file d’attente avec les actions de rejet ou de confirmation manuelle. Aucune confirmation n’a été effectuée, car aucune preuve de paiement réelle n’existait. La demande, le profil et le compte Auth du demandeur QA, ainsi que le compte administrateur QA, ont tous été supprimés par Appwrite avec HTTP 204. Aucun abonnement de production n’a été activé pendant ce contrôle.

## Session IndexedDB et navigation — 24 août 2026

Un compte indépendant QA a été inscrit depuis l’interface de production, puis son espace personnel a été chargé exclusivement avec les collections Appwrite vides du compte. Le retour explicite depuis l’espace vers l’accueil a conservé la session : l’accueil a affiché l’état **Connecté** et le bouton **Mon espace**. Le retour vers l’espace a restauré le même utilisateur sans présentation du formulaire de connexion. Un rechargement du navigateur a également conservé la session active.

L’instantané IndexedDB ne contient que les métadonnées non sensibles du profil ; il ne contient ni mot de passe, ni jeton, ni cookie. L’authentification réelle continue de dépendre de la session Appwrite. En cas d’indisponibilité réseau temporaire, l’instantané évite une redirection abusive ; une réponse Appwrite explicite 401 reste le seul signal qui nettoie la session locale. Le compte QA et son profil ont été supprimés après la vérification.

## Messagerie : lecture explicite et contrôle production — 24 août 2026

Le déploiement Appwrite `6a8c499a10d6c352dd36` de la Function `messaging` ajoute l’action sécurisée `read`. La consultation (`list`) reste désormais sans écriture ; l’interface appelle explicitement `read` lorsqu’une conversation devient active. Cette séparation élimine le décalage précédemment observé entre le message visible et le compteur de non-lus.

Le commit `e686de7` a été publié sur `main`, puis le déploiement Vercel de production correspondant est devenu `READY`. Deux comptes QA UY1 / ICT4D / L1 éphémères ont ouvert une conversation dans `#/app/messages`, envoyé et reçu un message depuis deux sessions navigateur distinctes. La conversation ouverte côté destinataire a affiché le message sans badge persistant ; une vérification serveur a ensuite confirmé le booléen de lecture du destinataire à `true`. La conversation, le message, les profils et les deux comptes Auth QA ont été supprimés immédiatement après ce contrôle.

La dernière suite `pnpm run test:e2e:uy1` s’est terminée avec `passed=true` et `cleanup.ok=true`. Elle couvre désormais le marquage lu explicite en plus de l’inscription, des dix inscriptions aux cours, de l’appel, des notes, de la demande WhatsApp manuelle et de l’intégrité. Les rafales `attendance_secure` ont toutes réussi : 1/1 à 17 208,45 ms côté client (3,50 s Function), 4/4 à p50 10 636,36 ms et p95 19 008,78 ms (p95 Function 3,40 s), et 8/8 à p50 13 668,60 ms et p95 25 866,81 ms (p95 Function 2,90 s). Les écarts client/Function restent attribuables aux opérations asynchrones et aux démarrages à froid ; les assertions se fondent donc sur les écritures Appwrite effectivement persistées.

## Présentation pédagogique ICT4D — 24 août 2026

Une présentation de 11 diapositives a été produite pour l’équipe pédagogique à partir des seules observations consignées : périmètre UY1 / ICT4D / L1, dix inscriptions aux cours, présence QR, notes pondérées, messagerie avec lecture explicite, demande WhatsApp manuelle, rafales E2E, session IndexedDB non sensible et limites déclarées. Le diaporama distingue explicitement les éléments validés des limites hors périmètre, notamment FCM distant, Stripe et toute extension à d’autres universités ou filières.
