...
/** Mêmes teintes que `typeColors`, en hex, pour les tuiles d'icônes du focus de journée. */
const TYPE_HEX: Record<string, string> = { CM: '#1E3A8A', TD: '#0D9488', TP: '#F97316', Projet: '#7C3AED' }
    ...
    <div className="space-y-5 animate-fade-in">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4 bg-gradient-to-r from-[#0f285f] via-[#1e3a8a] to-[#0d9488] px-5 py-5 text-white">
          <div><p className="flex items-center gap-1.5 text-xs font-black uppercase tracking-[0.16em] text-cyan-100"><UniIcon name="schedule" weight="fill" size={16} /> UniFlow · Appwrite</p><h1 className="mt-1 text-2xl font-black">Emploi du temps</h1><p className="mt-1 text-sm text-blue-100">{currentRole === 'teacher' ? 'Vos cours programmés' : `Grille ${currentUser.scopeLabel || 'de votre filière'} synchronisée depuis Appwrite`}</p></div>
          <div className="flex flex-wrap items-center gap-2"><button onClick={() => setWeekOffset((value) => value - 1)} aria-label="Semaine précédente" className="rounded-lg border border-white/25 bg-white/10 p-2 text-white hover:bg-white/20"><UniIcon name="chevronLeft" weight="bold" size={16} /></button><span className="rounded-lg bg-white/10 px-3 py-2 text-sm font-bold">{weekRangeLabel}</span><button onClick={() => setWeekOffset((value) => value + 1)} aria-label="Semaine suivante" className="rounded-lg border border-white/25 bg-white/10 p-2 text-white hover:bg-white/20"><UniIcon name="chevronRight" weight="bold" size={16} /></button>{weekOffset !== 0 && <button onClick={() => setWeekOffset(0)} className="rounded-lg bg-white px-3 py-2 text-xs font-black text-[#1e3a8a] hover:bg-blue-50">Aujourd’hui</button>}<button onClick={refetch} className="flex items-center gap-1.5 rounded-lg border border-white/25 bg-white/10 px-3 py-2 text-xs font-semibold text-white hover:bg-white/20"><UniIcon name="refresh" weight="bold" size={16} /> Actualiser</button></div>
        </div>
        <div className="grid divide-y divide-slate-100 sm:grid-cols-3 sm:divide-x sm:divide-y-0"><div className="flex items-center gap-3 p-4"><IconTile name="schedule" color="#1E3A8A" variant="filled" size={44} index={0} /><div><p className="text-lg font-black text-slate-900">{schedules?.length ?? 0}</p><p className="text-xs font-semibold text-slate-500">créneaux Appwrite</p></div></div><div className="flex items-center gap-3 p-4"><IconTile name="time" color="#0D9488" variant="filled" size={44} index={1} /><div><p className="text-lg font-black text-slate-900">{Math.round(totalMinutes / 60)} h</p><p className="text-xs font-semibold text-slate-500">charge hebdomadaire</p></div></div><div className="flex items-center gap-3 p-4"><IconTile name="agenda" color="#7C3AED" variant="filled" size={44} index={2} /><div><p className="text-sm font-black text-slate-900">{focusedDay?.label ?? 'Aucun jour'}</p><p className="text-xs font-semibold text-slate-500">{focusEntries.length} créneau(x) · {Math.round((focusMinutes / 60) * 10) / 10} h</p></div></div></div>
      </section>
      ...
      {learnerWithoutScope && (
        <section role="status" className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          <UniIcon name="students" size={20} className="mt-0.5 shrink-0 text-amber-600" />
          <div>
