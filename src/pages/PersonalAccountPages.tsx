...
export default function PersonalAccountPage({ kind }: { kind: PersonalAccountPageKind }) {
  ...
  const unavailable: Record<Exclude<PersonalAccountPageKind, 'profile' | 'settings'>, { title: string; description: string; icon: UniIconName }> = {
    messages: { title: 'Messagerie personnelle non exposée', description: 'Le backend personnel actuel ne fournit pas encore de route de messagerie pour les comptes indépendants. Aucun contenu fictif n’est affiché ici.', icon: 'messages' },
    library: { title: 'Bibliothèque personnelle non exposée', description: 'Le backend personnel actuel ne fournit pas encore de stockage documentaire ou de route de bibliothèque pour les comptes indépendants.', icon: 'library' },
    attendance: { title: 'Présences non applicables hors affiliation', description: 'Le suivi des présences nécessite une session universitaire et une liste d’étudiants affiliés. Cette fonctionnalité reste volontairement indisponible pour un compte indépendant.', icon: 'attendance' },
    ...
    video: { title: 'Visioconférence non exposée', description: 'La visioconférence dépend actuellement des services universitaires. Elle n’est pas activée pour les comptes indépendants.', icon: 'video' },
    classrooms: { title: 'Salles universitaires non accessibles', description: 'Les salles et ressources sont liées à une université. Les comptes indépendants peuvent gérer leurs propres lieux dans les créneaux de planning.', icon: 'room' },
    help: { title: 'Aide personnelle en cours de connexion', description: 'La FAQ et les tickets du backend universitaire ne sont pas utilisés dans un compte indépendant. Pour le moment, consultez votre profil, vos paramètres ou les offres personnelles sans données simulées.', icon: 'help' },
  }
...
export function PersonalSettingsPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6 pb-12"><PageHeader eyebrow="Espace indépendant" title="Paramètres" description="Des réglages adaptés à votre compte personnel. Les préférences d’affichage sont locales à cet appareil ; vos données académiques restent gérées par le backend personnel." icon={<UniIcon name="settings" size={24} />} />{notice && <Alerts notice={notice} clearNotice={() => setNotice(null)} />}<div className="grid gap-6 lg:grid-cols-2"><SettingsCard title="Compte" icon="profile"><div className="flex items-center justify-between gap-4"><div><p className="text-sm font-bold text-slate-900 dark:text-white">{currentUser.name}</p><p className="text-xs text-slate-500">{currentUser.email}</p></div><button type="button" onClick={() => navigate('/app/profil')} className="rounded-xl bg-teal-50 px-3 py-2 text-xs font-bold text-teal-800 hover:bg-teal-100">Modifier le profil</button></div><div className="mt-4 flex items-center gap-2 rounded-xl bg-slate-50 p-3 text-xs text-slate-500 dark:bg-slate-800"><UniIcon name="security" weight="fill" size={16} className="text-emerald-600" /> Session protégée par JWT.</div></SettingsCard><SettingsCard title="Connexion et synchronisation" icon={isOfflineMode ? 'wifiOff' : 'wifi'}><SettingRow label="Mode hors ligne" description="Suspendre les appels réseau jusqu’à votre reconnexion." control={<button type="button" onClick={() => setIsOfflineMode(!isOfflineMode)} className={`relative h-6 w-11 rounded-full transition ${isOfflineMode ? 'bg-rose-500' : 'bg-emerald-500'}`} aria-pressed={isOfflineMode}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow transition ${isOfflineMode ? 'left-6' : 'left-1'}`} /></button>} /><div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-900">Le mode hors ligne empêche les nouvelles lectures et écritures jusqu’à sa désactivation. Aucune donnée fictive n’est injectée.</div></SettingsCard><SettingsCard title="Apparence et sound design" icon={soundMuted ? 'soundOff' : 'sound'}><SettingRow label="Sons de feedback" description="Sons courts pour les actions réussies ou les erreurs. Aucun son n’est joué automatiquement." control={<button type="button" data-sound-ignore="true" onClick={() => saveSound(!soundMuted)} className={`relative h-6 w-11 rounded-full transition ${soundMuted ? 'bg-slate-300' : 'bg-teal-600'}`} aria-label={soundMuted ? 'Activer les sons' : 'Désactiver les sons'} aria-pressed={!soundMuted}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow transition ${soundMuted ? 'left-1' : 'left-6'}`} /></button>} /><div className="mt-3"><label className="flex items-center justify-between gap-4 text-xs font-bold text-slate-600 dark:text-slate-300"><span>Volume des feedbacks</span><span>{Math.round(soundVolume * 500)}%</span></label><input data-sound-ignore="true" type="range" min="0" max="0.2" step="0.01" value={soundVolume} onChange={event => saveSound(soundMuted, Number(event.target.value))} className="mt-2 w-full accent-teal-600" aria-label="Volume des sons de feedback" /></div><SettingRow label="Langue" description="Langue de l’interface UniFlow." control={<select value={language} onChange={event => { setLanguage(event.target.value as 'FR' | 'EN'); setNotice('Langue mise à jour.') }} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs font-bold dark:border-slate-700 dark:bg-slate-900"><option value="FR">Français</option><option value="EN">English</option></select>} /><SettingRow label="Réduire les animations" description="Préférence d’accessibilité enregistrée sur cet appareil." control={<button type="button" onClick={() => saveInterface(!reducedMotion)} className={`relative h-6 w-11 rounded-full transition ${reducedMotion ? 'bg-teal-600' : 'bg-slate-300'}`} aria-pressed={reducedMotion}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow transition ${reducedMotion ? 'left-6' : 'left-1'}`} /></button>} /></SettingsCard><SettingsCard title="Abonnement et sécurité" icon="billing"><Link to="/pricing" className="flex items-center justify-between rounded-xl bg-teal-50 p-3 text-xs font-bold text-teal-800 hover:bg-teal-100"><span className="flex items-center gap-2"><UniIcon name="billing" size={16} /> Voir les offres personnelles</span><UniIcon name="forward" weight="bold" size={14} /></Link><div className="mt-3 flex items-center gap-3 rounded-xl border border-slate-100 p-3 text-xs text-slate-500 dark:border-slate-800"><UniIcon name="key" size={16} className="text-slate-400" /> La gestion du mot de passe est disponible via le flux d’authentification sécurisé.</div></SettingsCard></div><div className="flex flex-wrap justify-between gap-3 rounded-3xl border border-rose-200 bg-rose-50 p-5"><div><p className="text-sm font-black text-rose-900">Quitter la session</p><p className="mt-1 text-xs text-rose-700">Les données enregistrées dans Neon ne sont pas supprimées.</p></div><button type="button" onClick={() => void logout()} className="inline-flex items-center gap-2 rounded-xl bg-rose-600 px-4 py-2.5 text-xs font-black text-white hover:bg-rose-700"><UniIcon name="logout" weight="bold" size={16} /> Se déconnecter</button></div></div>
}

export function PersonalUnavailablePage({ title, description, icon = 'help' }: { title: string; description: string; icon?: UniIconName }) { return <div className="mx-auto flex min-h-[70vh] max-w-3xl items-center justify-center py-10"><div className="w-full rounded-[2rem] border border-slate-200 bg-white p-8 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="mx-auto flex justify-center"><IconTile name={icon} color="#0D9488" variant="soft" size={56} /></div><p className="mt-6 text-xs font-black uppercase tracking-[0.2em] text-teal-700">État réel du service</p><h1 className="mt-3 text-2xl font-black text-slate-900 dark:text-white">{title}</h1><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-500">{description}</p><div className="mt-6 flex flex-wrap justify-center gap-3"><Link to="/app" className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-3 text-xs font-black text-white hover:bg-teal-700 dark:bg-white dark:text-slate-900"><UniIcon name="back" weight="bold" size={16} /> Retour à mon espace</Link><Link to="/app/profil" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-3 text-xs font-black text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200"><UniIcon name="profile" size={16} /> Mon profil</Link></div></div></div> }
...
function Alerts({ error, notice, clearError, clearNotice }: { error?: string | null; notice?: string | null; clearError?: () => void; clearNotice?: () => void }) { return <div className="space-y-3">{error && <div className="flex items-start gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800"><UniIcon name="alert" weight="fill" size={20} className="mt-0.5 shrink-0" /><p className="flex-1">{error}</p>{clearError && <button type="button" onClick={clearError} aria-label="Fermer"><UniIcon name="close" weight="bold" size={16} /></button>}</div>}{notice && <div className="flex items-start gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800"><UniIcon name="success" weight="fill" size={20} className="mt-0.5 shrink-0" /><p className="flex-1">{notice}</p>{clearNotice && <button type="button" onClick={clearNotice} aria-label="Fermer"><UniIcon name="close" weight="bold" size={16} /></button>}</div>}</div> }
function SettingsCard({ title, icon, children }: { title: string; icon: UniIconName; children: React.ReactNode }) { return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center gap-3 border-b border-slate-100 pb-4 dark:border-slate-800"><IconTile name={icon} color="#0D9488" variant="soft" size={36} /><h2 className="font-black text-slate-900 dark:text-white">{title}</h2></div><div className="pt-4">{children}</div></section> }
...
function PageLoading({ label }: { label: string }) { return <div className="flex min-h-[60vh] items-center justify-center gap-3 text-sm text-slate-500"><UniIcon name="spinner" weight="bold" size={28} className="animate-spin text-teal-600" />{label}</div> }
