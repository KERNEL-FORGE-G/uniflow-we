...
const emptyCourse: CourseForm = { code: '', title: '', instructor: '', credits: '', colorHex: '#0d9488', classroom: '', description: '' }
const emptySchedule: ScheduleForm = { courseId: '', dayOfWeek: '', startTime: '', endTime: '', classroom: '', type: '' }
const emptyAssignment: AssignmentForm = { courseId: '', title: '', dueDate: '', description: '', priority: '', status: '' }
const emptyGrade: GradeForm = { courseId: '', evaluationTitle: '', score: '', maxScore: '', coefficient: '' }
...
export default function IndependentWorkspacePage({ initialTab, scheduleOnly = false }: { initialTab?: Tab; scheduleOnly?: boolean }) {
  ...
  const handleCourseSubmit = async (event: FormEvent) => {
    ...
    const { credits, ...courseFields } = courseForm
    const payload = { ...courseFields, code: courseForm.code.trim().toUpperCase(), title: courseForm.title.trim(), ...(credits === '' ? {} : { credits }) }
  ...
  const handleScheduleSubmit = async (event: FormEvent) => {
    event.preventDefault()
    if (!scheduleForm.courseId || !scheduleForm.dayOfWeek || !scheduleForm.startTime || !scheduleForm.endTime || !scheduleForm.type || scheduleForm.startTime >= scheduleForm.endTime) return setError('Renseignez la matière, le jour, les heures et le type de séance.')
    await runMutation(async () => {
      if (editingSchedule) {
  ...
  const tabs: Array<{ id: Tab; label: string; icon: UniIconName; count: number }> = [
    { id: 'courses', label: 'Matières', icon: 'courses', count: courses.length },
    { id: 'schedule', label: 'Planning', icon: 'schedule', count: schedules.length },
    { id: 'assignments', label: 'Tâches', icon: 'tasks', count: assignments.length },
    { id: 'grades', label: 'Notes', icon: 'grades', count: grades.length },
  ]
    ...
    <div className="mx-auto max-w-[1500px] space-y-6 pb-12">
      <section className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#0f766e] via-[#0d9488] to-[#1e3a8a] p-6 text-white shadow-xl sm:p-8">
        ...
        <div className="relative flex flex-col gap-7 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-teal-100"><UniIcon name="assistant" weight="fill" size={16} /> Espace indépendant</div><h1 className="mt-3 text-3xl font-black tracking-tight sm:text-4xl">Votre parcours, vos données, votre rythme.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-teal-50">UniFlow vous donne un espace académique autonome. Les indicateurs ci-dessous sont calculés uniquement à partir de vos matières, créneaux, tâches et notes enregistrés dans votre compte personnel.</p></div>
          <div className="flex flex-wrap gap-2"><button type="button" onClick={() => beginCreate('courses')} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-xs font-extrabold text-teal-800 shadow-lg transition hover:-translate-y-0.5"><UniIcon name="plus" weight="bold" size={16} /> Ajouter une matière</button><button type="button" onClick={() => void loadData()} className="inline-flex items-center gap-2 rounded-xl border border-white/30 bg-white/10 px-4 py-2.5 text-xs font-extrabold text-white backdrop-blur transition hover:bg-white/20"><UniIcon name="refresh" weight="bold" size={16} /> Actualiser</button></div>
        </div>
      </section>

      <InlineAlert tone="success" icon={<UniIcon name="success" weight="fill" size={20} />} message="Les données de cet espace sont lues et enregistrées directement dans Appwrite pour votre compte personnel." />
      {error && <InlineAlert tone="error" icon={<UniIcon name="alert" weight="fill" size={20} />} message={error} onClose={() => setError(null)} />}
      {notice && <InlineAlert tone="success" icon={<UniIcon name="success" weight="fill" size={20} />} message={notice} onClose={() => setNotice(null)} />}

      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard icon="courses" index={0} label="Matières suivies" value={courses.length} detail="dans votre espace" tone="teal" /><MetricCard icon="time" index={1} label="Tâches en retard" value={overdueAssignments.length} detail={overdueAssignments.length ? 'à traiter rapidement' : 'aucune échéance dépassée'} tone={overdueAssignments.length ? 'amber' : 'emerald'} /><MetricCard icon="trophy" index={2} label="Moyenne personnelle" value={weightedAverage == null ? '—' : `${weightedAverage.toFixed(2)}/20`} detail={grades.length ? `${grades.length} évaluation(s)` : 'ajoutez vos premières notes'} tone="blue" /><MetricCard icon="agenda" index={3} label="Créneaux planifiés" value={schedules.length} detail={todaySchedules.length ? `${todaySchedules.length} aujourd’hui` : 'aucun créneau aujourd’hui'} tone="violet" /></section>

      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]"><div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-wider text-teal-600">Vue d’ensemble</p><h2 className="mt-1 text-xl font-black text-slate-900 dark:text-white">Gérez votre espace</h2></div><div className="relative w-full sm:w-64"><UniIcon name="search" weight="bold" size={16} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Filtrer mes données…" className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-teal-500 focus:bg-white dark:border-slate-700 dark:bg-slate-800" /></div></div><div className="mt-5 flex flex-wrap gap-2 border-b border-slate-100 pb-4 dark:border-slate-800">{tabs.map(({ id, label, icon, count }) => <button key={id} type="button" onClick={() => { setTab(id); setSearch(''); closeForms() }} className={`inline-flex items-center gap-2 rounded-xl px-3.5 py-2.5 text-xs font-bold transition ${tab === id ? 'bg-slate-900 text-white shadow-md dark:bg-white dark:text-slate-900' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800'}`}><UniIcon name={icon} weight={tab === id ? 'fill' : 'bold'} size={16} />{label}<span className={`rounded-full px-2 py-0.5 text-[10px] ${tab === id ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-800'}`}>{count}</span></button>)}</div><div className="mt-5">{tab === 'courses' && <CoursesPanel courses={filteredCourses} editing={editingCourse} form={courseForm} setForm={setCourseForm} saving={saving} onSubmit={handleCourseSubmit} onCancel={closeForms} onEdit={course => { setEditingCourse(course); setCourseForm({ code: course.code, title: course.title, instructor: course.instructor ?? '', credits: course.credits ?? 0, colorHex: course.colorHex ?? '#0d9488', classroom: course.classroom ?? '', description: course.description ?? '' }) }} onDelete={handleCourseDelete} />}{tab === 'schedule' && <SchedulePanel schedules={filteredSchedules} courses={courses} editing={editingSchedule} form={scheduleForm} setForm={setScheduleForm} saving={saving} onSubmit={handleScheduleSubmit} onCancel={closeForms} onEdit={item => { setEditingSchedule(item); setScheduleForm({ courseId: item.courseId, dayOfWeek: item.dayOfWeek, startTime: item.startTime, endTime: item.endTime, classroom: item.classroom ?? '', type: item.type ?? 'CM' }) }} onDelete={handleScheduleDelete} />}{tab === 'assignments' && <AssignmentsPanel assignments={filteredAssignments} courses={courses} editing={editingAssignment} form={assignmentForm} setForm={setAssignmentForm} saving={saving} onSubmit={handleAssignmentSubmit} onCancel={closeForms} onEdit={item => { setEditingAssignment(item); setAssignmentForm({ courseId: item.courseId, title: item.title, dueDate: item.dueDate ? item.dueDate.slice(0, 16) : '', description: item.description ?? '', priority: item.priority ?? 'MEDIUM', status: item.status ?? 'TODO' }) }} onDelete={handleAssignmentDelete} />}{tab === 'grades' && <GradesPanel grades={filteredGrades} courseById={courseById} editing={editingGrade} form={gradeForm} setForm={setGradeForm} saving={saving} onSubmit={handleGradeSubmit} onCancel={closeForms} onEdit={item => { setEditingGrade(item); setGradeForm({ courseId: item.courseId, evaluationTitle: item.evaluationTitle, score: String(item.score), maxScore: String(item.maxScore), coefficient: String(item.coefficient) }) }} onDelete={handleGradeDelete} />}</div></div><aside className="space-y-6"><SubscriptionStatus /><TodayCard schedules={todaySchedules} courseById={courseById} /><UrgentTasks tasks={overdueAssignments.length ? overdueAssignments : assignments.filter(item => item.status !== 'DONE' && item.status !== 'CANCELLED').slice(0, 4)} courseById={courseById} onOpen={() => setTab('assignments')} /><GradeChart grades={grades} /></aside></section>
    </div>
  )
}

function LoadingState() { return <div className="mx-auto flex min-h-[65vh] max-w-5xl items-center justify-center"><div className="flex flex-col items-center gap-3 text-sm text-slate-500"><UniIcon name="spinner" weight="bold" size={36} className="animate-spin text-teal-600" /><span>Chargement de votre espace personnel…</span></div></div> }
function InlineAlert({ tone, icon, message, onClose }: { tone: 'warning' | 'error' | 'success'; icon: ReactNode; message: string; onClose?: () => void }) { const classes = tone === 'warning' ? 'border-amber-200 bg-amber-50 text-amber-900' : tone === 'error' ? 'border-rose-200 bg-rose-50 text-rose-800' : 'border-emerald-200 bg-emerald-50 text-emerald-800'; return <div className={`flex items-start gap-3 rounded-2xl border p-4 text-sm ${classes}`}><span className="mt-0.5 shrink-0">{icon}</span><p className="flex-1 leading-6">{message}</p>{onClose && <button type="button" onClick={onClose} aria-label="Fermer" className="opacity-70 hover:opacity-100"><UniIcon name="close" weight="bold" size={16} /></button>}</div> }
/** Couleurs pleines des tuiles de statistiques (la spec veut une tuile 56 px en dégradé, plus un fond pastel). */
const METRIC_TONES = { teal: '#0D9488', amber: '#D97706', emerald: '#059669', blue: '#1E3A8A', violet: '#7C3AED' } as const
function MetricCard({ icon, index = 0, label, value, detail, tone }: { icon: UniIconName; index?: number; label: string; value: string | number; detail: string; tone: keyof typeof METRIC_TONES }) { return <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-800 dark:bg-slate-900"><div className="flex items-start justify-between gap-3"><IconTile name={icon} color={METRIC_TONES[tone]} variant="filled" size={56} index={index} /><span className="text-2xl font-black text-slate-900 dark:text-white">{value}</span></div><p className="mt-4 text-xs font-bold text-slate-600 dark:text-slate-300">{label}</p><p className="mt-1 text-[11px] text-slate-400">{detail}</p></div> }
function TodayCard({ schedules, courseById }: { schedules: PersonalSchedule[]; courseById: Map<string, PersonalCourse> }) { return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between"><div><p className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Aujourd’hui</p><h3 className="mt-1 font-black text-slate-900 dark:text-white">Mon planning</h3></div><UniIcon name="agenda" size={20} className="text-teal-600" /></div>{schedules.length === 0 ? <p className="mt-5 rounded-xl bg-slate-50 p-4 text-xs leading-5 text-slate-500 dark:bg-slate-800">Aucun créneau personnel enregistré pour aujourd’hui.</p> : <div className="mt-4 space-y-2">{schedules.slice().sort((a, b) => a.startTime.localeCompare(b.startTime)).map((item, index) => { const course = courseById.get(item.courseId); const title = item.courseTitle ?? course?.title ?? 'Matière supprimée'; return <div key={item.id} className="flex items-center gap-3 rounded-xl bg-teal-50/70 p-3 dark:bg-teal-950/30"><IconTile subject={title} subjectCode={item.courseCode ?? course?.code} color={subjectColor(item.courseCode ?? course?.code, course?.colorHex)} variant="soft" size={36} index={index} /><span className="w-20 shrink-0 text-xs font-black text-teal-800 dark:text-teal-300">{item.startTime}–{item.endTime}</span><span className="min-w-0 truncate text-xs font-bold text-slate-800 dark:text-slate-200">{title}</span></div> })}</div>}</section> }
function UrgentTasks({ tasks, courseById, onOpen }: { tasks: PersonalAssignment[]; courseById: Map<string, PersonalCourse>; onOpen: () => void }) { return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between"><div><p className="text-[11px] font-bold uppercase tracking-wider text-amber-600">Priorités</p><h3 className="mt-1 font-black text-slate-900 dark:text-white">Tâches à surveiller</h3></div><button type="button" onClick={onOpen} aria-label="Ouvrir les tâches" className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700"><UniIcon name="forward" weight="bold" size={16} /></button></div>{tasks.length === 0 ? <p className="mt-5 rounded-xl bg-slate-50 p-4 text-xs leading-5 text-slate-500 dark:bg-slate-800">Aucune tâche en attente. Ajoutez vos prochaines échéances pour les suivre ici.</p> : <div className="mt-4 space-y-2">{tasks.slice(0, 4).map(task => <div key={task.id} className="rounded-xl border border-slate-100 p-3 dark:border-slate-800"><div className="flex items-start justify-between gap-3"><p className="truncate text-xs font-bold text-slate-800 dark:text-slate-200">{task.title}</p><span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${isOverdue(task) ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}>{isOverdue(task) ? 'En retard' : statusLabels[task.status ?? 'TODO']}</span></div><p className="mt-1 text-[11px] text-slate-500">{courseById.get(task.courseId)?.code ?? 'Sans matière'} · {task.dueDate ? formatDate(task.dueDate, true) : 'Sans échéance'}</p></div>)}</div>}</section> }
function GradeChart({ grades }: { grades: PersonalGrade[] }) { return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between"><div><p className="text-[11px] font-bold uppercase tracking-wider text-blue-600">Progression</p><h3 className="mt-1 font-black text-slate-900 dark:text-white">Vos dernières notes</h3></div><UniIcon name="grades" size={20} className="text-blue-600" /></div>{grades.length === 0 ? <p className="mt-5 rounded-xl bg-slate-50 p-4 text-xs leading-5 text-slate-500 dark:bg-slate-800">Aucune note personnelle enregistrée.</p> : <div className="mt-5 space-y-3">{grades.slice(0, 5).map(grade => { const percentage = Math.max(0, Math.min(100, (Number(grade.score) / Math.max(Number(grade.maxScore), 1)) * 100)); return <div key={grade.id}><div className="mb-1 flex items-center justify-between gap-3 text-[11px]"><span className="truncate font-bold text-slate-700 dark:text-slate-300">{grade.evaluationTitle}</span><span className="font-black text-blue-700">{grade.score}/{grade.maxScore}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800"><div className="h-full rounded-full bg-gradient-to-r from-blue-600 to-teal-500 transition-all" style={{ width: `${percentage}%` }} /></div></div> })}</div>}</section> }
function SectionEmpty({ title, description, icon = 'filter' }: { title: string; description: string; icon?: UniIconName }) { return <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50/70 p-10 text-center dark:border-slate-700 dark:bg-slate-800/50"><div className="mx-auto flex justify-center"><IconTile name={icon} color="#0D9488" variant="soft" size={56} /></div><h3 className="mt-4 text-sm font-black text-slate-800 dark:text-slate-200">{title}</h3><p className="mx-auto mt-2 max-w-md text-xs leading-5 text-slate-500">{description}</p></div> }
function CardActions({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) { return <div className="flex items-center gap-1"><button type="button" onClick={onEdit} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-teal-700" title="Modifier" aria-label="Modifier"><UniIcon name="edit" weight="bold" size={16} /></button><button type="button" onClick={onDelete} className="rounded-lg p-2 text-slate-400 hover:bg-rose-50 hover:text-rose-600" title="Supprimer" aria-label="Supprimer"><UniIcon name="trash" weight="bold" size={16} /></button></div> }
function CoursesPanel({ courses, editing, form, setForm, saving, onSubmit, onCancel, onEdit, onDelete }: { courses: PersonalCourse[]; editing: PersonalCourse | null; form: CourseForm; setForm: (value: CourseForm) => void; saving: boolean; onSubmit: (event: FormEvent) => void; onCancel: () => void; onEdit: (course: PersonalCourse) => void; onDelete: (course: PersonalCourse) => void }) { return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_320px]"><div className="space-y-3">{courses.length === 0 ? <SectionEmpty icon="courses" title="Aucune matière personnelle" description="Commencez par enregistrer une matière pour l’utiliser dans votre planning, vos tâches et vos notes." /> : courses.map((course, index) => <div key={course.id} className="rounded-2xl border border-slate-200 p-4 transition hover:border-teal-200 hover:shadow-sm dark:border-slate-800"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-3"><IconTile subject={course.title} subjectCode={course.code} color={subjectColor(course.code, course.colorHex)} variant="filled" size={44} index={index} /><div className="min-w-0"><p className="text-[11px] font-black uppercase tracking-wider text-teal-700">{course.code}</p><h3 className="mt-1 truncate font-black text-slate-900 dark:text-white">{course.title}</h3><p className="mt-1 text-xs text-slate-500">{course.instructor || 'Enseignant non renseigné'} · {course.credits ?? 0} crédit(s){course.classroom ? ` · ${course.classroom}` : ''}</p></div></div><CardActions onEdit={() => onEdit(course)} onDelete={() => onDelete(course)} /></div>{course.description && <p className="mt-3 rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-600 dark:bg-slate-800 dark:text-slate-300">{course.description}</p>}</div>)}</div><CourseForm form={form} setForm={setForm} editing={Boolean(editing)} saving={saving} onSubmit={onSubmit} onCancel={onCancel} /></div> }
const SCHEDULE_GRID_START_HOUR = 7
const SCHEDULE_GRID_END_HOUR = 21
const SCHEDULE_GRID_ROW_HEIGHT = 56
...
function WeeklyScheduleGrid({ schedules, courses, onEdit, onDelete }: { schedules: PersonalSchedule[]; courses: PersonalCourse[]; onEdit?: (item: PersonalSchedule) => void; onDelete?: (item: PersonalSchedule) => void }) {
  ...
  const hours = Array.from({ length: SCHEDULE_GRID_END_HOUR - SCHEDULE_GRID_START_HOUR }, (_, index) => SCHEDULE_GRID_START_HOUR + index)
  const courseById = new Map(courses.map(course => [course.id, course]))

  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="border-b border-slate-200 px-4 py-3 dark:border-slate-800">
        <p className="text-sm font-black text-slate-900 dark:text-white">Grille hebdomadaire</p>
        <p className="mt-1 text-xs text-slate-500">Les créneaux affichés proviennent directement de votre Planning personnel Appwrite.</p>
      </div>
      <div className="overflow-x-auto">
        <div className="min-w-[980px]">
          <div className="grid border-b border-slate-200 bg-slate-50/80 dark:border-slate-800 dark:bg-slate-800/40" style={{ gridTemplateColumns: '68px repeat(7, minmax(130px, 1fr))' }}>
            <div className="border-r border-slate-200 dark:border-slate-800" />
            {days.map(day => <div key={day} className="border-r border-slate-200 px-3 py-3 text-center last:border-r-0 dark:border-slate-800"><p className="text-[11px] font-black uppercase tracking-wider text-slate-700 dark:text-slate-200">{dayLabels[day]}</p></div>)}
          </div>
          <div className="grid" style={{ gridTemplateColumns: '68px repeat(7, minmax(130px, 1fr))' }}>
            <div className="border-r border-slate-200 dark:border-slate-800">
              {hours.map(hour => <div key={hour} className="relative border-b border-slate-100 pr-2 text-right text-[10px] font-bold text-slate-400 dark:border-slate-800" style={{ height: SCHEDULE_GRID_ROW_HEIGHT }}><span className="relative -top-2">{String(hour).padStart(2, '0')}:00</span></div>)}
            </div>
            {days.map(day => {
              const dayItems = schedules.filter(item => item.dayOfWeek === day).sort((a, b) => a.startTime.localeCompare(b.startTime))
              return (
                <div key={day} className="relative border-r border-slate-200 last:border-r-0 dark:border-slate-800" style={{ height: gridHeight }}>
                  {hours.map(hour => <div key={hour} className="border-b border-slate-100 dark:border-slate-800" style={{ height: SCHEDULE_GRID_ROW_HEIGHT }} />)}
                  {dayItems.map(item => {
                    ...
                    const height = Math.max((itemEnd - itemStart) * pixelsPerMinute - 4, 42)
                    const title = item.courseTitle ?? course?.title ?? 'Matière supprimée'
                    const color = subjectColor(item.courseCode ?? course?.code, course?.colorHex)
                    return (
                      <article key={item.id} className="absolute left-1 right-1 overflow-hidden rounded-lg border border-white/70 p-2 text-white shadow-sm" style={{ top, height, backgroundColor: color }}>
                        <div className="flex items-start justify-between gap-1">
                          <div className="min-w-0"><p className="flex items-center gap-1 truncate text-[10px] font-black"><SubjectIcon subject={title} code={item.courseCode ?? course?.code} weight="fill" size={12} className="shrink-0" />{title}</p><p className="mt-0.5 text-[9px] font-semibold text-white/85">{item.startTime}–{item.endTime}</p></div>
...
function CourseForm({ form, setForm, editing, saving, onSubmit, onCancel }: { form: CourseForm; setForm: (value: CourseForm) => void; editing: boolean; saving: boolean; onSubmit: (event: FormEvent) => void; onCancel: () => void }) { return <FormCard title="une matière" editing={editing} saving={saving} onSubmit={onSubmit} onCancel={onCancel}><div className="grid grid-cols-2 gap-3"><Field label="Code" value={form.code} required onChange={value => setForm({ ...form, code: value })} /><Field label="Crédits" type="number" value={form.credits} onChange={value => setForm({ ...form, credits: value === '' ? '' : Number(value) })} /></div><Field label="Nom de la matière" value={form.title} required onChange={value => setForm({ ...form, title: value })} /><Field label="Enseignant" value={form.instructor} onChange={value => setForm({ ...form, instructor: value })} /><Field label="Semestre ou salle" value={form.classroom} onChange={value => setForm({ ...form, classroom: value })} /><Field label="Description" value={form.description} onChange={value => setForm({ ...form, description: value })} /></FormCard> }
